...
class Strconvert:
    def __init__(self, string):
        self.str1 = string.strip()
        if '-' or '—' in self.str1:
            self.str1 = ''
        elif 'mL' in self.str1:
            self.str1 = self.str1.replace('mL', '')
...
class Handler(BaseHandler):
    ...
    def detail_page(self, response):
        ...
        if '￥' in jiage:
            ...
            v_jiage = -99
        yuanjia = response.doc('span.cd_m_info_fyb').text()
        if '新车' in yuanjia:
            yuanjia2 = yuanjia.replace('新车', '')
            v_yuanjia = int(float(yuanjia2.replace('万（含税）', ''))*10000)
        else:
            v_yuanjia = -99
        ...
        zhengbeizhiliang = response.doc(
            '#cd_m_clxx > div.cd_m_i_pz > dl:nth-child(2) > dd:nth-child(6) > span.cd_m_i_pz_val').text()
        if 'kg' in zhengbeizhiliang:
            v_zhengbeizhiliang = int(zhengbeizhiliang.replace(' kg', ''))
        else:
            v_zhengbeizhiliang = -99
        zhouju = response.doc(
                '#cd_m_clxx > div.cd_m_i_pz > dl:nth-child(2) > dd:nth-child(7) > span.cd_m_i_pz_val').text()
        if 'mm' in zhouju:
            v_zhouju = int(zhouju.replace(' mm', ''))
        else:
            v_zhouju = -99
        pailiang = response.doc(
                '#cd_m_clxx > div.cd_m_i_pz > dl:nth-child(3) > dd:nth-child(4) > span.cd_m_i_pz_val').text()
        v_pailiang2 = Strconvert(pailiang)
        v_pailiang = v_pailiang2.tofloat()

        paiqiliang = response.doc(
                'div.cd_m_pop_pzcs_content > div > ul > li:nth-child(3) > dl > dd:nth-child(21) > span.cd_m_pop_pzcs_val').text()
        v_paiqiliang2 = Strconvert(paiqiliang)
        v_paiqiliang = v_paiqiliang2.tofloat()
        ...
        zongheyouhao = response.doc(
                '#cd_m_clxx > div.cd_m_i_pz > dl:nth-child(3) > dd:nth-child(7) > span.cd_m_i_pz_val').text()
        if 'L' in zongheyouhao:
            v_zongheyouhao = float(zongheyouhao.replace(' L/100km', ''))
        else:
            v_zongheyouhao = -99

        zuidagonglv = response.doc(
                'div.cd_m_pop_pzcs_content > div > ul > li:nth-child(3) > dl > dd:nth-child(9) > span.cd_m_pop_pzcs_val').text()
        v_zuidagonglv2 = Strconvert(zuidagonglv)
        v_zuidagonglv = v_zuidagonglv2.tofloat()
